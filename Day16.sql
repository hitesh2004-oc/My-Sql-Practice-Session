-- select name from employee as e join deptsalary as  ds where ds.salary < e.salary;
-- select name from employee as e join deptsalary as  ds where e.salary > (select avg(salary) from dept where departments ='IT');
...
-- Correlated SubQuery
select emp.ename, emp.salary,emp.department_name from employee as emp
where emp.salary > 
(select avg(ein.salary) from employee as ein where ein.department_name = emp.department_name);

-- CTE[Common table Expression]
with xyz as 
(select department_name,avg(salary) as salary from employee group by department_name)

select employee.ename,employee.salary,employee.department_name from employee join 
xyz on employee.department_name = xyz.department_name
