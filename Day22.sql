use world;
select * from country;
select code,name,population, avg(population) over() from country;
select continent,sum(population) from country group by continent;

select code,name,continent, sum(population) over(),sum(population) over(partition by continent) from country;
select name,continent,max(population) over(),max(population) 
...
(107, 'Pooja Shah',  'HR',         'Mumbai',    60000, '2020-02-01'), (108, 'Arjun Iyer',  'HR',         'Chennai',   58000, '2022-06-18');
...
(11,102, '2026-02-10', 16000.00), (12,103, '2026-02-12',  4000.00);
...
-- Running Salary fro Each department 
...
-- rank() -> Give the ranking particular columns and skip rank
select *,rank() over(order by salary) from employees;
select *,rank() over(order by city) from employees;

-- dense_rank() -> Give the ranking particular columns and not skip rank
